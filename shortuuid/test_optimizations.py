...
class TestOptimizationCorrectness:
    ...
    @pytest.mark.parametrize("padding", [None, 10, 22, 50])
    @pytest.mark.parametrize(
        "number", [0, 1, 255, 65535, 2**32 - 1, 2**64 - 1, 2**128 - 1]
    )
    def test_int_to_string_identical_results(
        self, number: int, alphabet: list[str], padding: int | None
    ):
    ...
    @pytest.mark.parametrize("alphabet", ALPHABETS)
    def test_string_to_int_identical_results(self, alphabet: list[str]):
    ...
    @pytest.mark.parametrize("alphabet", ALPHABETS)
    def test_get_alphabet_identical_results(self, alphabet: list[str]):
        ...
        optimized = su_unsorted.get_alphabet()
        assert original == optimized
    ...
    def test_valueerror_preserved(self):
        ...
        with pytest.raises(ValueError, match="'x' is not in alphabet"):
...
def benchmark_int_to_string() -> tuple[float, float]:
...
def benchmark_string_to_int() -> tuple[float, float, float]:
...
def benchmark_get_alphabet() -> tuple[float, float]:
...
def measure_memory_overhead() -> dict[str, int]:
...
if __name__ == "__main__":
    ...
    for alphabet in ALPHABETS:
        for padding in [None, 10, 22, 50]:
            for number in [0, 1, 255, 65535, 2**32 - 1, 2**64 - 1, 2**128 - 1]:
                test.test_int_to_string_identical_results(number, alphabet, padding)
    print("  ✓ int_to_string produces identical results")

    for alphabet in ALPHABETS:
        test.test_string_to_int_identical_results(alphabet)
    print("  ✓ string_to_int produces identical results")

    for alphabet in ALPHABETS:
        test.test_get_alphabet_identical_results(alphabet)
